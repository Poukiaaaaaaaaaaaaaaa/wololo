...
function preload() { //chargement des images
  ...
  img.piece.noir[4] = loadImage("img/Pieces/cavalier_noir.png") // cavalier noirrrrrr
...
class Piece {
	//classe repr�sentant une pi�ce en général
  ...
  viewRanges() {
    ...
    var board = examineBoard();  //récupération du tableau repr�sentant l'�chiquier
    var depl = this.getDepl(board); //récupération de la liste des cases o� il est possible de de d�placer
									//la m�thode getDepl est définie dans chaque classe de pièce, le d�placement �tant propre � celle-ci
...
function mouseClicked(){
  if (mouseButton == LEFT){
    ...
    for (let i = 0; i < chessGUI.windows.length; i++) {
      if (chessGUI.windows[i].shouldClose()) chessGUI.windows.plice(i, 1);
    }
  }
}
