var titleView = { //Objet contenant plusieurs fonctions : chacune sert à initialiser une "page" de l'écran titre : elles créent les éléments à afficher pour chaque page
	...
	settings : function(){ //Page de configuration
		...
		let miniBoard = new p55_object("hud", 0, 0,
			config.canvasW/4,config.unit * 44
		)
		miniBoard.coef = 0.5
		...
		for (i = 0; i < joueur.length; i++){
			for (let j = 0; j < joueur[i].prePiece.length; j++){
				...
				currentPiece = (currentPre.Piece.constructor.name == "Array") ? currentPre.Piece[currentPre.selected] : currentPre.Piece
				let butno = new Button("pieceHUD", 
					(currentPiece.img) ? ((currentPiece.img.__proto__.name == "Piece") ? img.piece[joueur[i].color][currentPiece.img] : currentPiece.img) : img.piece[joueur[i].color][currentPiece],
								...
								for (j = 0; j < this.prePiece.Piece.length % 5; j++){
									...
									elements[i].push(
										{type: "button", coord: {x: config.unit * 4 + j * (config.unit * 4 + config.tileSize), y: config.unit * 4, w: config.tileSize, h: config.tileSize},
											img: (currentPiece.img) ? ((currentPiece.img.__proto__.name == "Piece") ? img.piece[this.color][currentPiece.img] : currentPiece.img ) : img.piece[this.color][currentPiece] 
										}
									)				
								}
							}
							console.log("y: "+elements[0][0].coord.y)
							console.log("winH: "+(config.tileSize + config.unit * 8))
							console.log("buttonH: "+elements[0][0].coord.h)
							let windowSize = ((this.prePiece.Piece.length > 5) ? 5 : this.prePiece.Piece.length) * (config.tileSize + config.unit * 4) + config.unit * 4
							let pieceWindow = new Window(config.canvasW / 2 - windowSize / 2, config.canvasH/4, windowSize, config.tileSize + config.unit * 8, "Select a piece Class",elements)
						}	
					}
				)	
