var titleView = { //Objet contenant plusieurs fonctions : chacune sert à initialiser une "page" de l'écran titre : elles créent les éléments à afficher pour chaque page
	...
	settings : function(){ //Page de configuration
		...
		if (game.isReady){
			...
			new Button("hud",img.title[8],
				miniBoard.x + drawBoard.getW()*miniBoard.coef+config.unit * 3,config.canvasH/4,
				config.unit * 10,config.unit*5,
				function(){fill([255,255,255,50]); rect(this.x, this.y, this.w, this.h)},
				function(){
					window.pieceLoaderPopup = window.open("lib/chess/pieceLoader.html");
					pieceLoaderPopup.update = function(){
						let list = pieceLoaderPopup.document.getElementById("pList");
						pieceLoaderPopup.document.getElementById("pList")
						for (piece in chessPP.Piece){
							if (chessPP.Piece.hasOwnProperty(piece)) list.innerHTML += '<li>'+chessPP.Piece[piece].name+'<br>'
						}
					}
					onTick.add(
						function(){
							if (window.pieceLoaderPopup.document.getElementById("pList") != null){
								window.pieceLoaderPopup.update()
								this.delete()
							}
						}
					)
