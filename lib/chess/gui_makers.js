			...
			new Button("hud",img.title[8],
				...
				function(){
					...
					pieceLoaderPopup.update = function(){
						let list = pieceLoaderPopup.document.getElementById("pList"), element
						pieceLoaderPopup.document.getElementById("pList")
						for (piece in chessPP.Piece){
							if (chessPP.Piece.hasOwnProperty(piece)) {
								element = list.appendChild(document.createElement('li'))
								element.innerHTML = chessPP.Piece[piece].name + '<br>'
							}
						}
					}
					pieceLoaderPopup.onload = function(){
						this.update()
					}
				}
			)
