...
function initPrePieces(layoutID) { //initialise les "prePieces" de chaque joueur selon la configuration de base des pièces d'échecs
	...
	for (let i = 0; i < layout.length; i++) {
		joueur[0].prePiece.push(new PrePiece(layout[i].piece, layout[i].x, layout[i].y,0));
		joueur[1].prePiece.push(new PrePiece(layout[i].piece, layout[i].x, config.nLig - 1 - layout[i].y,1));
	}		
	return true ;
}
