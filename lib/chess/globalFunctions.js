function onceComplete(){ //éxécutée lorsque tous les fichiers ont été chargés
	...
	classBuilder = Function(classBuilders[i])()
		...
		chessPP.Piece[classBuilder.name].default = true 
	}
	let r = initPrePieces(0); //crée leurs prePieces de base
	
	if (r) game.isReady = true
}

function initPrePiecesLayout(layoutID){
	let layout = chessPP.pieceLayout[layoutID]
	if (!(layout && layout.isLayout)) return -1
	
	for (let i = 0; i < layout.length; i++){
		if (!layout[i].piece) {console.log("Error in config.json : the piece n° " + i + "doesn't have a proper piece id"); return -2} else 
		if (typeof layout[i].piece == "string"){
			if (!chessPP.Piece[layout[i].piece]) return -2;
			layout[i].piece = chessPP.Piece[layout[i].piece]
		} else if (layout[i].piece.constructor.name == "Array") {
			for (let j = 0; j < layout[i].piece.length; j++){
				if (!chessPP.Piece[layout[i].piece[j]]) {layout[i].piece.slice(j, 1) ; j -= 1 ; continue}
				layout[i].piece[j] = chessPP.Piece[layout[i].piece[j]]
			}
			if (layout[i].piece.length == 0) return -2
		}
	}
	return layout
}

function initPrePieces(layoutID) { //initialise les "prePieces" de chaque joueur selon la configuration de base des pièces d'échecs
	...
	let r = initPrePiecesLayout(layoutID)
	if (r < 0 ) {
		if (r == -1 ) console.error("Tentative de charger une configuration inexistante (id : " + layoutID + ")")
		if (r == -2) console.error("Une ou plusieurs pièces de la configuration sélectionnée n'ont pu être chargées")
		return false;	
	}	
	chessPP.prePieceLayout = r;
	
	let layout = chessPP.prePieceLayout;
	for (let i = 0; i < layout.length; i++) {
		joueur[0].prePiece.push(new PrePiece(layout[i].piece, layout[i].x, layout[i].y,0, layout[i].selected));
		joueur[1].prePiece.push(new PrePiece(layout[i].piece, layout[i].x, config.nLig - 1 - layout[i].y,1, layout[i].selected));
	}		
	return true ;
}
...
var deleteItem = function(object, item){
	delete this[getPropertyName(object, item)];
}
...
Number.prototype.max = function(m){
...
Number.prototype.min = function(m){
	let num = this.valueOf()
	if (num < m) {
		return m
	} else {
...
Number.prototype.markOut = function(min, max){
	return this.valueOf().min(min).max(max);
}
...
function kill(target,killer){ //tue une pièce
	...
	if (killer && killer.player) joueur[killer.player].gold += target.goldValue;
...
function heal(target,source,heal = 0,type = null){
	var parameters = {source : source, heal : heal, target : target, type: type};
	if (target.callPassive("onHealed",parameters) == true) return true //si l'un des passifs pré-heal renvoie true
	if (source.callPassive("onHealing",parameters) == true) return true //le heal sont annulés et la fonction renvoie elle aussi true (permet à un passif d'annuler un heal)
	target.hp += heal
	if (target.hp > target.maxHP) target.hp = target.maxHP
	
	target.callPassive("onHealedDone",parameters)
	source.callPassive("onHealingDone",parameters)
...
function drawBoard(x = 0, y = 0, h = config.nLig, w = config.nCol, tileSize = config.tileSize, border = config.border) {
 //dessine case par case l'échiquier
  for (var i = 0; i < w; i++) {
    for (var j = 0; j < h; j++) {
      if ((i + j) % 2 == 0) { fill(50); } else { fill(255); }
      rect(i*tileSize + (i + 1)*border + x,
           j*tileSize + (j + 1)*border + y,
           tileSize,
           tileSize,
           border);
    }
...
drawBoard.getW = function(w = config.nCol){
	return w * config.tileSize + (w-1) * config.border
}

drawBoard.getH = function(w = config.nLig){
	return h * config.tileSize + (h-1) * config.border
}
...
function casesInLineS(cx,cy,direction,l, includeOrigin = true){
	let dx = (direction % 2 == 0) ? 1 - direction : 0
	let dy = (direction % 2 == 1) ? direction - 2 : 0
	let cases = []
	let i = (includeOrigin) ? 0 : 1; 
	
	for (i; i < l; i++){
		if (isOnBoard(cx + dx * i, cy + dy * i)) cases.push([cx + dx * i, cy + dy * i])
	}
...
function addDomElement(node,element,attributes,refNode){	
	if (refNode) node.insertBefore(element,refNode)
		else newElement = node.appendChild(element);
	if (!newElement) return false
	if (typeof attributes == "object") Object.assign(newElement,attributes);
	chessGUI_DOM.push(newElement);
	return newElement;
}

function delDomElement(domElement){
	if (domElement && domElement.parentNode){ domElement.parentNode.removeChild(domElement);
	chessGUI_DOM.spliceItem(domElement)
	return true } else {return false}
}

function clearChessDomGUI(){
	for (let i = 0; i < chessGUI_DOM.length; i++){
		delDomElement(chessGUI_DOM[i])
	}
}

function isPiece(obj){
	return (obj.constructor.__proto__.name == "Piece")	
}

function loadJSON_(file ,callback) {
...
var EventHandlerPrototype = { //on définit ici des props à ajouter au prototype de chaque fonction ajoutée à l'event
		enabled : true,
		delete : function(){this.event.spliceItem(this)},
		enable : function(){this.enabled = true},
		disable : function(){this.enabled = false}
		
}

function Event_(name = "Event"){
	
	let eventList = []
	
	eventList.name = name
	eventList.add = function(fct){
		if (typeof fct != "function"){
			console.error("The value or reference you tried to add in event " + this.name + "is not a correct event handler. Only functions can be used as event handlers")
		}
		fct.event = this;
		Object.assign(fct, EventHandlerPrototype) //ajout de handlerProto au prototype de la fonction
		
		this.push(fct)
	}
	
	eventList.process = function(){
		for (let i = 0; i < this.length; i++){
			if (this[i].enabled === true){
				this[i]()
			}
		}
	}
	
	eventList.delete = function(handler){
		this.spliceItem(handler);	
	}
