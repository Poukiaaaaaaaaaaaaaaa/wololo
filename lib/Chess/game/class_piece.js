class Piece {
	...
	constructor(name,atk,hp,cx,cy,player,mp,expValue = 0, goldValue = 0, image = this.constructor.img) { //On ne créera jamais d'instances de cette classe directement : ce sont les classes
		...
		this.mp = mp; //stat de point de déplacements
		...
		this.img = (image) ? ((image.__proto__.name == "Piece") ? img.piece[this.color][image] : image) : img.piece[this.color][this.constructor]
  ...
  //méthode affichant la pièce
    if (!(this.movement)) {this.x = convertPx(this.cx) ; this.y = convertPx(this.cy)} //Si la pièce n'est pas en mouvement (=ne possède pas actuellement d'attribut mouvement), sa position est calculée
      image(this.img, //Affiche l'image de la pièce
  ...
  noManaError(x,y){ //Affiche, à une position spécifiée, un message d'erreur "not enough mana"
	...
	// let files = ["pion.js","tour.js","fou.js","cavalier.js","reine.js","roi.js","priest.js"];
	// var classBuilders = [];
	...
	// for (let i = 0; i < files.length; i++){
		...
		// xobj.onreadystatechange = function() {
			// if (xobj.readyState == 4 && xobj.status == "200") {
				// classBuilders[i] = xobj.responseText;
				// loadCount += 1
				// console.log("ooeeoeooeoeooe")
			// }
		// }
		// xobj.send(null);
...
include("lib/chess/game/piece/priest.js");
...
loadJSON_("lib/chess/game/piece/config.json",
	function(rep){
		...
		window.prePieceLayout = configObj
		for (let i = 0; i < prePieceLayout.length; i++){
			...
			if (typeof prePieceLayout[i].piece == "string"){
				prePieceLayout[i].piece = window[prePieceLayout[i].piece]
			} else if (prePieceLayout[i].piece.constructor.name == "Array") {
				for (let j = 0; j < prePieceLayout[i].piece.length; j++){
					console.log(prePieceLayout[i].piece[j])
					prePieceLayout[i].piece[j] = window[prePieceLayout[i].piece[j]]
				}
				
			}
		}
		console.log(prePieceLayout)
	}
)
...
-- Ajouter l'objet pieceClass à ??? (via Function)
- Décider de la manière dont on gère les pieceClasses (array, global, etc)
- Gérer plusieurs config
