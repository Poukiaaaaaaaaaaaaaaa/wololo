class Piece {
	...
	constructor(name,atk,hp,cx,cy,player,mp,expValue,img) { //On ne créera jamais d'instances de cette classe directement : ce sont les classes
		...
		this.img = img || img.piece[this.color][this.constructor]
  ...
  //méthode affichant la pièce
    if (!(this.movement)) {this.x = convertPx(this.cx) ; this.y = convertPx(this.cy)} //Si la pièce n'est pas en mouvement (=ne possède pas actuellement d'attribut mouvement), sa position est calculée
      image(img.piece[this.color][this.constructor], //Affiche l'image de la pièce
