...
class SpellIcon extends Button { //icône des spells; hérite des simples boutons
		...
		function(){ //callback du bouton :
			if (guiState == ""){ //si la GUI est à son état normal (aucune opération particulière en cours)
				if(joueur[this.spell.piece.player].mana >= this.spell.manaCost){ //si le joueur a assez de mana
					if (this.spell.actualCooldown == 0 && !this.spell.locked && !this.spell.piece.cc && (!this.spell.condition || this.spell.condition())){ //et si le spell n'est pas en récupération
	...
	draw(){ //Affiche l'icône
		...
		if (this.spell.actualCooldown || this.spell.locked || (this.spell.condition && !this.spell.condition())){ //si le spell est bloqué ou en récupération, le grise
			fill([0,0,0,150])
